...
public class SpringStarter<P, Q extends IHandler<P>> {
    ...
    private ExecutorModule executorGroup;
    ...
    private EStatus status = EStatus.RUNNING;
    ...
    public SpringStarter<P, Q> executorModule(ExecutorModule executorModule) {
        this.executorGroup = executorModule;
        return this;
    }

    public SpringStarter<P, Q> groupContext(Object groupContext) {
    ...
    public void sync(long timeout) {
        ...
        log.info("[{}]并发模块 任务总数({})，起始任务({}) ", executorGroup.getModuleName(), undoTasks.get(), headList.size());
    ...
    private void submitTask(TaskNode taskNode) {
        ...
        if (pre == 0) {
            executorGroup.getExecutor().submit(() -> {
                try {
                    log.info("[{}]开始执行 待完成任务数量({})", taskNode.getTaskName(), undoTasks.get());
                    taskNode.getHandler().doHandler(context);
                } catch (Exception e) {
                    log.error("[{}]出现异常 {}", taskNode.getTaskName(), e);
                    taskNode.getHandler().onError(e);
                } finally {
                    finishTask(taskNode);
                }
            });
    ...
    private void finishTask(TaskNode taskNode) {
        if (status != EStatus.RUNNING) {
            log.info("[{}]执行中断({})，开始唤醒主线程...", executorGroup.getModuleName(), status);
